...
A mediano plazo podemos considerar únicamente a los centros urbanos, la población más expuerta al virus. Según el [nuevo diario](https://www.elnuevodiario.com.ni/economia/463377-mas-mitad-nicas-residen-areas-urbanas/) al 2018 había un 57.6% de la población viviendo en concentraciones urbanas. Esto es, la población en riesgo de contagio corresponde a `r format(57.6/100 * 6632977, scientific = FALSE, big.mark=",")` personas, lo que resultaría en una mortalidad a mediano plazo de 49,819.98 * 57.6/100 = **`r format(49819.98 * 57.6/100, scientific = FALSE, big.mark=",")` individuos**.
...
Según diario de Nicaragua, [El 19 Digital](https://www.el19digital.com/articulos/ver/titulo:101277-vigilancia-epidemiologica-permanente-permite-que-nicaragua-aun-no-registre-casos-de-coronavirus), no existen casos de COVID-19 en el país al 15 de marzo de 2020. Los [protocolos de prevensión usados usados](https://www.el19digital.com/el19tv/ver/titulo:14699-asi-es-el-protocolo-de-seguridad-sanitaria-ante-el-coronavirus-en-el-aeropuerto-augusto-c-sandino), no son infalibles. Basta con observar lo [casos de COVID-19 en los paises vecinos de centroamérica](https://nuevaya.com.ni/centroamerica-reporta-dos-fallecidos-por-el-coronavirus/) para entender que a estas alturas es altamente probable que existan personas infectadas que no han sido identificadas.

Hasta el momento según el [SICA](https://nuevaya.com.ni/centroamerica-reporta-dos-fallecidos-por-el-coronavirus/), al 15 de marzo de 2020, en Centroamérica se han identificado los siguientes casos:
...
Debido a que la base de datos es grande y difícil de manipular manualmente usando hojas de cálculo, hacemos uso de `R`, uno de los software de cienca de datos más populares al día de hoy, para extraer la base de datos y calcular el promedio de crecimiento de todos los países registrados en la OMS:
...
\begin{equation}
N(t) = \frac{(3,820,595)(14)}{(3,820,595 - 14)e^{-0.347t}+14},
\end{equation}
...
El modelo inicia al tiempo cero, con 14 casos el día 15 de Marzo. Aproximadamente para el 4 de Abril se espera un fuerte crecimiento en el número de casos que se mantendrá a lo largo de un mes, cuando la saturación en la población haga que la infección se ralentice. La mayoría de los casos de infección serán "inofensivos", sin embargo, un 0.75% serán letales e impondrá un fuerte estrés al sistema de salud. Se espera que entre el 4 de Abril y el 4 de Mayo el sistema de salud del país colapse a menos que se tomen medida precautorias, que incluso pueden probar no ser suficientes.
...
Sí, sí se puede. Existen diversas estrategias para combatir la infección que han probado ser efectivas para reducir el crecimiento de la infección. Cabe notar que el avance del COVID-19 hace no factible evitar el contagio de la enfermedad aún en cuarentenas. Entre los países que han logrado mitigar el avance de las infecciones se encuentra el Korea del Sur, que ha dado un ejemplo para el mundo, y de manera local [Costa Rica](https://www.ministeriodesalud.go.cr/index.php/centro-de-prensa/noticias/741-noticias-2020/1572-gobierno-declara-estado-de-emergencia-nacional-impide-llegada-de-extranjeros-y-se-suspende-lecciones-en-todos-los-centros-educativos-del-pais).

Con sus medidas Korea del Sur ha logrado mantener una [tasa de crecimiento que dobla los casos confirmados en 11 días](https://ourworldindata.org/coronavirus). Esto implica una tasa diaria de crecimiento de $2^{1/11}$ = `r format(2^(1/11), digits=4)`. Es decir, que en un día los casos en Korea del Sur crecen en un $r$ de `r format((2^(1/11)-1)*100, digits = 3)` %. Por otro lado, siguiendo la [misma fuente de información](https://ourworldindata.org/coronavirus) del caso de Korea del Sur, Costa Rica mantiene una tasa de crecimiento inicial que dobla los casos confirmados en 6 días. Esto implica una tasa diaria de $2^{1/6}$ = `r format(2^(1/6), digits = 4)`, o un $r$ de `r format((2^(1/6)-1)*100, digits = 3)`%.

Si Nicaragua tomara las medidas de Korea del Sur o de Costa Rica, los casos que requerirían hospitalización se reducirían de la siguiente manera:
...
abline(h = 5781, lty = 2)
legend("topright", legend = c("Sin distanciamiento social", "Con distanciamiento tipo Korea del Sur", "Con distanciamiento tipo Costa Rica", "Camas disponibles en el país"), lty = c(1,5,3,2))
```

Claramente se observa que el efecto de tomar medidas como las de Korea del Sur deja al país en una situación grave, pero todavía manejable, con una esperanza de salir airosos de la crisis. El total de casos no se disminuye, pero poder controlar la tasa de crecimiento evita el colapso del sistema de salud.

## Para Concluir, ¿Qué Podemos Hacer por Nuestro País?
...
Actuar con prontitud y contundencia parace ser el curso recomendado. Postergar las acciones o implementarlas inadecuadamente puede llevar a [graves consecuencias](https://www.thelancet.com/journals/lancet/article/PIIS0140-6736(20)30522-5/fulltext).
